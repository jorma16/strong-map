...
test('Get truthy keys returns all the keys of a map with value is true', () => {
  const strongMap = new StrongMap([['True', true], [{ is: 'True' }, true], ['False', false]]);
  const truthyKeys = strongMap.getTruthyKeys();

  expect(truthyKeys).toEqual(['True', { is: 'True' }]);
});
